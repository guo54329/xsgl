...
<div class="panel panel-default">
	  ...
	  <div class="form-inline">
		<form action='<?php echo U(GROUP_NAME.'/Excise/courseTable');?>' method="post">
			...
			<button type="submit" class="btn btn-info" name='export' value='export'><span class='glyphicon glyphicon-download'></span> 导出</button>　
	  		方式二：
		 ...
		 <table class="table table-bordered table-hover tablesorter" style="font-size: 11px;">
		  <thead>
			<tr style="text-align: center;font-weight: bold;">
				<th width="50px" style="text-align: center;">ID</th><th width="100px" style="text-align: center;">学期</th><th  width="70px" style="text-align: center;">教师</th><th style="text-align: center;">班级</th><th width="100px"  style="text-align: center;">课程</th><td width="60px">任务数</td><td>下发/提交</td><th width="60px" style="text-align: center;">完成率</th><th  style="text-align: center;" min-width="80px">文件</th><td width="190px">操作(要删除请先删任务)</td></tr>
			</thead>
			<tbody id="tob">
			...
			<?php if(is_array($coursetable)): foreach($coursetable as $key=>$v): $tr = substr($v['term'],10,1); ?>
    			...
    			<td><?php $i=0; $recnum = M('sxsetcourse as a')->join('inner join xh_sxpubexcise as b on a.scid = b.scid')->join('inner join xh_sxsubexcise as c on b.peid=c.peid')->where("a.scid=$scid")->count(); if($recnum>0)$i=1; ?>
    			    ...
    			    /
    			     <?php $i=0; $subnum = M('sxsetcourse as a')->join('inner join xh_sxpubexcise as b on a.scid = b.scid')->join('inner join xh_sxsubexcise as c on b.peid=c.peid')->where("a.scid=$scid and c.status=1")->count(); if($subnum>0)$i=1; if($recnum>0){ $finish = (round($subnum / $recnum,4)*100)."%"; }else{ $finish = '0%'; } $j=0; if($subnum==$recnum and $subnum!=0){ $j=1; } ?>
				...
				<td align="left">　
				   <a href="<?php echo U(GROUP_NAME.'/Excise/sxpubexciseList',array('scid'=>$v['scid']));?>" class="btn btn-default btn-sm" title="查看指定教师发布的指定课程的任务列表！"><span class="glyphicon glyphicon-eye-open"></span> 任务</a>&nbsp;
					...
					<?php if($pubnum == 0): ?><a href="<?php echo U(GROUP_NAME.'/Excise/delcourseTable',array('id'=>$v['scid']));?>" class="btn btn-default btn-sm" title=""><span class="glyphicon glyphicon-remove"></span> 删除</a><?php endif; ?>
				</td>
			</tr>
