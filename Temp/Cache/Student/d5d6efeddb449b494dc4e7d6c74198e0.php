		 ...
		 <table class="table table-bordered table-hover tablesorter" style="font-size: 11px;">
		   <thead>
			...
			<?php if(is_array($list)): foreach($list as $key=>$v): ?><tr>
				...
				<td><?php if($v['status'] == 0 ): ?><span style="font-size: 14px;color:red;">待完成</span><?php else: ?><img src="__PUBLIC__/Images/finish.png" style="height: 28px;" /><?php endif; ?></td>
				<td>
					<?php if($v['filea']!=''){ $file = $v['url'].$v['filea']; echo format_bytes(filesize($file)); } ?>
				</td>
				<td><?php echo $v['desc']*0.3+$v['isrec']*0.7; ?></td>
			
				<td>
					<?php if($v['status'] == 0 ): ?><a href="<?php echo U(GROUP_NAME.'/Excise/sxsubexciseDesc',array('seid'=>$v['seid']));?>" class="btn btn-default" style="font-size: 14px;color: red;"><span class="glyphicon glyphicon-hand-right"></span> 去完成</a>
					<?php else: ?>
						<a href="<?php echo U(GROUP_NAME.'/Excise/sxsubexciseDownAttach',array('seid'=>$v['seid']));?>" class="btn btn-default btn4 btn4"><span class="glyphicon glyphicon-save"></span>作业下载</a>&nbsp;
						<a href="<?php echo U(GROUP_NAME.'/Excise/sxexciseDiscuss',array('peid'=>$v['peid']));?>" class="btn btn-default btn2" ><span class="glyphicon glyphicon-eye-open"></span> 交流</a><?php endif; ?>
				</td>
			</tr>
